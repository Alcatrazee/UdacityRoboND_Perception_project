...
PR2PickPlace::PR2PickPlace(ros::NodeHandle nh)
    : nh_(nh), right_move_group(RIGHT_PLANNING_GROUP),
  ...
  right_joint_model_group =
      right_move_group.getCurrentState()->getJointModelGroup(
          RIGHT_PLANNING_GROUP);
  left_joint_model_group =
      left_move_group.getCurrentState()->getJointModelGroup(
          LEFT_PLANNING_GROUP);
  right_gripper_joint_model_group =
      right_gripper_group.getCurrentState()->getJointModelGroup(
          RIGHT_GRIPPER_GROUP);
  left_gripper_joint_model_group =
      left_gripper_group.getCurrentState()->getJointModelGroup(
          LEFT_GRIPPER_GROUP);
  ...
  visual_tools_ptr->publishText(
      text_pose, "Welcome to Advance Pick and Place project",
...
bool PR2PickPlace::Routine(pr2_robot::PickPlace::Request &req,
  ...
  if (!success) {
    ...
    std::string TARGET_MESH_PATH = OBJECT_MESH_PATH + req.object_name.data +
                                   "/meshes/" + req.object_name.data + ".dae";

    SetupCollisionObject(req.object_name.data, TARGET_MESH_PATH, act_obj_pose,
    ...
    if (req.arm_name.data == "right") {
      ...
      visual_tools_ptr->publishText(text_pose, "Reach Pose",
                                    rviz_visual_tools::WHITE,
                                    rviz_visual_tools::XLARGE);
      visual_tools_ptr->publishTrajectoryLine(right_arm_plan.trajectory_,
                                              right_joint_model_group);
      visual_tools_ptr->trigger();
      ...
      visual_tools_ptr->publishText(text_pose, "Pick Pose",
                                    rviz_visual_tools::WHITE,
                                    rviz_visual_tools::XLARGE);
      visual_tools_ptr->publishTrajectoryLine(right_arm_plan.trajectory_,
                                              right_joint_model_group);
      ...
      visual_tools_ptr->publishText(text_pose, "Drop Pose",
                                    rviz_visual_tools::WHITE,
                                    rviz_visual_tools::XLARGE);
      visual_tools_ptr->publishTrajectoryLine(right_arm_plan.trajectory_,
                                              right_joint_model_group);
      ...
      visual_tools_ptr->publishText(text_pose, "Reach Pose",
                                    rviz_visual_tools::WHITE,
                                    rviz_visual_tools::XLARGE);
      visual_tools_ptr->publishTrajectoryLine(left_arm_plan.trajectory_,
                                              left_joint_model_group);
      visual_tools_ptr->trigger();
      visual_tools_ptr->prompt("Click Next");
...
bool PR2PickPlace::IsPickPoseWithinLimits(geometry_msgs::Pose &pick_pose,
  ...
  dist_sq = (act_obj_pose.position.x - pick_pose.position.x) *
                (act_obj_pose.position.x - pick_pose.position.x) +
            (act_obj_pose.position.y - pick_pose.position.y) *
                (act_obj_pose.position.y - pick_pose.position.y) +
            (act_obj_pose.position.z - pick_pose.position.z) *
                (act_obj_pose.position.z - pick_pose.position.z);
...
bool PR2PickPlace::SetupCollisionObject(
...
bool PR2PickPlace::OperateRightGripper(const bool &close_gripper) {
  ...
  gripper_current_state->copyJointGroupPositions(
      right_gripper_joint_model_group, gripper_joint_positions);
...
int main(int argc, char **argv) {
  ...
  ros::ServiceServer service = nh.advertiseService(
      "pick_place_routine", &PR2PickPlace::Routine, &pr2_pick_place);
  ros::waitForShutdown();
  return 0;
}
